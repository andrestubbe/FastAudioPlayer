...
- **â±ï¸ Ultra-Low Latency**: Direct Windows WASAPI Exclusive/Shared mode access via JNI with native COM initialization.
- **âš™ï¸ Zero GC Overhead**: Optimized playbacks using lightweight native state-handles (Zero-GC ring buffers).
- **ðŸ“¦ Zero External Dependencies**: Just requires Java 17+ and Windows. Bundles pre-compiled DLLs.
- **ðŸŽ›ï¸ Total Audio Control**: Real-time Volume, Pause, Resume, Stop, playback position queries, and output device selection.
